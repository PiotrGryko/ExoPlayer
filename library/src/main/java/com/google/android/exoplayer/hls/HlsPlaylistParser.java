...
public final class HlsPlaylistParser implements UriLoadable.Parser<HlsPlaylist>
{
    ...
    private static HlsMediaPlaylist currentList;
            ...
            throws IOException
    {
        ...
        HlsMediaPlaylist tmpList = new HlsMediaPlaylist(baseUri, mediaSequence, targetDurationSecs, version, false,
                segments);
        ...
        if (currentList == null)
            ...
            addNewPlaylist(currentList, tmpList);

        return currentList;
        //return tmpList;
    }

    private static void addNewPlaylist(HlsMediaPlaylist oldList, HlsMediaPlaylist mediaPlaylist)
    {


        long durationToAdd = 0;
        for (int i = 0; i < mediaPlaylist.segments.size(); i++) {
            HlsMediaPlaylist.Segment s = mediaPlaylist.segments.get(i);

            boolean contains = false;
            for (int j = 0; j < oldList.segments.size(); j++) {
                if (s.url.equals(oldList.segments.get(j).url)) {
                    contains = true;
                    break;
                }
            }
            ...
            long segmentDuration = (long) s.durationSecs * 1000000;
            Segment newSegment = new HlsMediaPlaylist.Segment(s.url, s.durationSecs, s.discontinuitySequenceNumber,
                    s.startTimeUs + oldList.durationUs + segmentDuration, s.isEncrypted, s.encryptionKeyUri, s.encryptionIV,
                    s.byterangeOffset, s.byterangeLength);
            oldList.segments.add(newSegment);
            durationToAdd += segmentDuration;
            Log.d("hlschunksource", "old list size after update " + oldList.segments.size() + " new list size " + mediaPlaylist.segments.size());

            Log.d("hlschunksource", "segment added starting time " + newSegment.startTimeUs + " segment url " + newSegment.url);


        }

        oldList.durationUs = oldList.durationUs + durationToAdd;
        oldList.targetDurationSecs = mediaPlaylist.targetDurationSecs;
    }
