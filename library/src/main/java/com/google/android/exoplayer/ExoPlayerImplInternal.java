...
/* package */ final class ExoPlayerImplInternal implements Handler.Callback
{
    ...
    {
        for (int i = 0; i < enabledRenderers.size(); i++) {
            long tmp = enabledRenderers.get(i).getDurationUs();
            if (tmp > durationUs)
                durationUs = tmp;

            //Log.d("zzz", "update duration us " +enabledRenderers.get(i).getDurationUs()+" class "+enabledRenderers.get(i).getClass().getName());
